...
class MTIntegration:
    ...
    def set_broadcast_fn(
        self, broadcast_fn: Callable[[str, Dict[str, Any]], Awaitable[None]]
    ) -> None:
        """Set the broadcast function for emitting events."""
        self._broadcast_fn = broadcast_fn

    async def translate_and_emit(
        self, text: str, segment_id: str, is_final: bool = True, src_lang: str = "en"
    ) -> None:
        ...
        try:
            ...
            result = self._mt_service.translate_text(
                text=text, src_lang=src_lang, tgt_lang=tgt_lang, quality="realtime"
            )
    ...
    async def translate_chunked_and_emit(
        self, chunks: list[str], segment_ids: list[str], src_lang: str = "en"
    ) -> None:
        ...
        try:
            ...
            results = list(
                self._mt_service.translate_chunked(
                    chunks=chunks, src_lang=src_lang, tgt_lang=tgt_lang, quality="realtime"
                )
            )
