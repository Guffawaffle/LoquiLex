...
class WSEnvelope(BaseModel):
    ...
    t_mono_ms: Optional[int] = Field(default=None, description="Monotonic milliseconds since session start")
...
class HeartbeatData(BaseModel):
    ...
    latency_ms_est: Optional[int] = Field(default=None, description="Estimated latency in milliseconds")
...
class SessionState:
    ...
    def get_replay_messages(self, last_seq: int) -> list[WSEnvelope]:
        ...
        return [
            env for seq, env in sorted(self.replay_buffer.items())
            if seq > last_seq
        ]
