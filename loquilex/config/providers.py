...
class HuggingFaceConfig:
    ...
    def __post_init__(self) -> None:
        ...
        if self.token and not (
            self.token.startswith("hf_") or len(self.token) in [34, 37]  # Standard HF token lengths
        ):
...
class ProvidersConfig:
    ...
    def from_env(cls) -> ProvidersConfig:
        ...
        return cls(
            huggingface=HuggingFaceConfig(token=hf_token, enabled=True),
            backend=BackendConfig(offline=_env_bool("LX_OFFLINE", False)),
        )
    ...
    def from_file(cls, config_path: Path) -> Optional[ProvidersConfig]:
        ...
        try:
            ...
            return cls(
                huggingface=HuggingFaceConfig(
                    token=hf_data.get("token"), enabled=hf_data.get("enabled", True)
                ),
                backend=BackendConfig(offline=backend_data.get("offline", False)),
            )
    ...
    def save_to_file(self, config_path: Path) -> None:
        ...
        data = {
            ...
            "backend": {"offline": self.backend.offline},
        }
