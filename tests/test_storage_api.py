...
def test_set_base_directory_valid_path(client):
    ...
    response = client.post("/storage/base-directory", json={"path": "/tmp/loquilex-test"})
    assert response.status_code == 200
...
def test_set_base_directory_invalid_path(client, tmp_path):
    ...
    # Skip if running as root (e.g., in Docker) since root can access 0o000 dirs
    if os.geteuid() == 0:
...
def test_set_base_directory_relative_path(client):
    ...
    response = client.post("/storage/base-directory", json={"path": "relative/path"})
    assert response.status_code == 200
...
def test_set_base_directory_missing_path(client):
    ...
    response = client.post("/storage/base-directory", json={})
