...
class TestWSProtocolManager:
    ...
    def test_custom_configuration(self):
        ...
        manager = WSProtocolManager("test_session", hb_config=hb_config, limits=limits)
    ...
    async def test_client_ack_handling(self):
        ...
        ack_envelope = WSEnvelope(t=MessageType.CLIENT_ACK, data=AckData(ack_seq=1).model_dump())
    ...
    async def test_resume_expired_error(self):
        ...
        hello_envelope = WSEnvelope(
            t=MessageType.CLIENT_HELLO,
            data=ClientHelloData(
                agent="test-client/1.0", resume=ResumeInfo(sid="test_session", last_seq=0)
            ).model_dump(),
        )
