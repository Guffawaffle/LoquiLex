...
def test_mt_integration_disabled():
    ...
    integration = MTIntegration(session_id="test-session", mt_enabled=False, dest_lang="zh-Hans")
...
def test_mt_integration_enabled():
    ...
    integration = MTIntegration(session_id="test-session", mt_enabled=True, dest_lang="zh-Hans")
...
async def test_mt_integration_translate_and_emit():
    ...
    mock_service.translate_text.return_value = Mock(
        text="你好", provider="mock-provider", quality="realtime", src_lang="en", tgt_lang="zh-Hans"
    )
    ...
    await integration.translate_and_emit(
        text="Hello", segment_id="seg001", is_final=True, src_lang="en"
    )

    # Verify service was called
    mock_service.translate_text.assert_called_once_with(
        text="Hello", src_lang="en", tgt_lang="zh-Hans", quality="realtime"
    )
